...
use ggez::{graphics, Context, GameResult, timer, input, event};
...
impl EventHandler for Pong {
    fn update(&mut self, context: &mut Context) -> GameResult<()> {
        ...
        if self.player_paddle_location.y < 0.0 {
            self.player_paddle_location.y = 0.0;
        } else if self.player_paddle_location.y + self.paddle_height > self.window_height {
            self.player_paddle_location.y = self.window_height - self.paddle_height;
        }
